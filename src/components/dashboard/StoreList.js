...
const StoreList = () => {
    ...
    const fetchStoreStats = async () => {
      try {
        const statsPromises = stores.map(store => 
          axios.get(`/Store/${store.id}/stats`)
            .then(response => ({ 
              storeId: store.id, 
              stats: response.data 
            }))
  ...
  // Get initial letter of store name
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`/Store/${editingStore.id}`, editForm);
      fetchStores(activeFilter);
          ...
          <div key={store.id} className="store-card">
            ...
            <div className="store-stats">
              ...
              <div className="stat-item">
                ...
                <div className="stat-value">{formatCurrency(storeStats[store.id]?.totalRevenue || 0)}</div>
              </div>
            </div>
